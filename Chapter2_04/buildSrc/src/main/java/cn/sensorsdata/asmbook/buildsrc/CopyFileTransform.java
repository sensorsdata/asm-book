...
public class CopyFileTransform extends Transform {
    ...
    /**
     * 此处返回的文件如果发生变化，根据 SecondaryFile 的设定，可以决定是否应该是增量编译，还是全量编译。
     * 例如返回的是：ImmutableList.of(SecondaryFile.incremental(file))，那么当此 file 发生变化的时候，可以通过 transformInvocation.getSecondaryInputs() 中获取到的
     * SecondaryInput 实例，此实例可以通过 getSecondaryInput() 获取到 SecondaryFile ，例如下面的代码示例：
     * <p>
     * Collection<SecondaryInput> secondaryInputs = transformInvocation.getSecondaryInputs();
     * System.out.println("======secondaryInputs===" + secondaryInputs.size());
     * secondaryInputs.stream().forEach(input -> {
     * SecondaryFile secondaryFile = input.getSecondaryInput();
     * Status status = input.getStatus();
     * System.out.println("===transform secondary file====" + secondaryFile.getFile() + "=====" + secondaryFile.supportsIncrementalBuild());
     * System.out.println("===transform secondary file status====" + status.name());
     * });
     */
    @Override
    public Collection<SecondaryFile> getSecondaryFiles() {
        File file = new File("/Users/zhangwei/Documents/work/others/book-asm/Chapter2_04/log.txt");
        System.out.println("getSecondaryFiles===" + file);
    ...
    public void transform(TransformInvocation transformInvocation) throws TransformException, InterruptedException, IOException {
        System.out.println("=====start transform====="+transformInvocation.isIncremental());
        //获取需要消费的数据
